...
namespace aoc_2017.Day14
{
    ...
    public class Knot
    {
        ...
        private readonly byte[] StdSuffix = { 17, 31, 73, 47, 23 };
        private List<byte> Bites { get; set; }
        private List<int> Lengths
        {
            get
            {
                return Bites.Select(bit => (int)bit).ToList();
            }
        }
        ...
        public Knot(int count, string input)
            : this(count)
        {
            Bites = Encoding.ASCII.GetBytes(input).ToList();
            Bites.AddRange(StdSuffix);
        }
